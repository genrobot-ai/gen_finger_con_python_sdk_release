...
class DASProtocol:
    ...
    def find_packet(cls, data: bytes) -> Tuple[List[bytes], bytes]:
        ...
        while len(buffer) - search_start >= cls.MAGIC_LENGTH * 2:
            ...
            if footer_pos == -1:
                remaining_data = buffer[header_pos:]
                cls._log_debug(
                    "not found footer from index: {}, keeping {} bytes for complete packet".format(footer_search_start, len(remaining_data))
                )
                break
            ...
            if footer_pos > len(buffer) - cls.MAGIC_LENGTH:
                remaining_data = buffer[header_pos:]
                cls._log_debug(
                    f"Bad footer position {footer_pos}, buffer len {len(buffer)}, keeping {len(remaining_data)} bytes"
                )
                break
            ...
            if (
                ...
                cls._log_debug("Consecutive footer magic; possible packet glue")

            packet_end = footer_pos + cls.MAGIC_LENGTH
            full_packet = buffer[header_pos:packet_end]

            if len(full_packet) > cls.MAX_PACKET_SIZE:
                cls._log_warning(f"Packet too large ({len(full_packet)} bytes), skipping")
                search_start = header_pos + cls.MAGIC_LENGTH
                continue
            if cls._validate_packet_structure(full_packet):
                ...
                cls._log_debug("Invalid packet structure, skipping byte")
                search_start = header_pos + cls.MAGIC_LENGTH
        else:
            remaining_data = buffer[search_start:]
        ...
        if packets:
            cls._log_debug(
                f"Found {len(packets)} packet(s), {len(remaining_data)} bytes left in buffer"
            )
    ...
    def _validate_packet_structure(cls, packet: bytes) -> bool:
        """Return True if framing looks valid."""
        try:
            ...
            cls._log_error(f"Packet structure validation error: {e}")
            return False

    @classmethod
    def parse_packet(cls, packet: bytes) -> Optional[Dict[str, Any]]:
        """Parse validated packet into fields."""
        try:
            ...
            cls._log_error(f"Packet parse error: {e}")
            return None
    ...
    def create_packet(cls, opcode: int, data: bytes = b"") -> bytes:
        try:
            if not isinstance(opcode, int) or opcode < 0 or opcode > 255:
                raise ValueError("opcode must be an integer 0–255")
...
class DASController:
    ...
    def _reset_buffer(self):
        ...
        self._log_warning("Resetting buffer")
        self.buffer = b""
        self.consecutive_errors = 0
    ...
    def _handle_packet_with_retry(self, packet: bytes, max_retries: int = 3):
        for attempt in range(max_retries):
            try:
                ...
                self._log_error(f"Packet handling error (attempt {attempt+1}): {e}")
    ...
    def _handle_valid_packet(self, parsed_packet: Dict):
        ...
        print(
            ...
            f"payload length: {parsed_packet['data_length']}"
        )
    ...
    def _handle_invalid_packet(self, packet: bytes, attempt: int):
        self._log_warning(f"Invalid packet (attempt {attempt+1}): len={len(packet)} bytes")

        if attempt == 0:
            self._log_debug(f"Invalid packet hex: {packet.hex()[:100]}...")
    ...
    def _handle_sensor_data(self, data: bytes):
        try:
            if len(data) >= 4:
                ...
                print(f"Sensor data - timestamp: {timestamp}, value: {sensor_value.hex()}")
        except Exception as e:
